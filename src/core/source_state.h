...
struct source_state {
  ...
  std::string get_target_name(const std::string &source_name) const;
  [[nodiscard]] std::shared_ptr<entry> locate_entry(const std::string& target_name) const;
  ...
  void walk(const std::function<void(std::shared_ptr<entry>)>& func);
